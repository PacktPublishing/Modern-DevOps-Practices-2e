...
def add_rating(review_id, rating):
    try:
        res = requests.post(f'{ratings_service_url}/review/{review_id}/rating/{rating}')
...
def update_rating(review_id, rating):
    try:
        res = requests.put(f'{ratings_service_url}/review/{review_id}/rating/{rating}')
...
def add_review(post_id):
    ...
    # Insert review here
    inserted_review = collection.insert_one({'post_id': post_id, 'review': review})
...
def update_review(post_id, review_id):
    ...
    rating = data.get('rating')
    review = collection.find_one_and_update({'_id': ObjectId(review_id)}, {'$set': {'review': review_content}}, return_document=True)
